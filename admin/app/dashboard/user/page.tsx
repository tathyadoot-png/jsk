...
import { adminUsersContent } from "@/modules/pages/user/adminUsers.content";
import {
...
export default function AdminUsersPage() {
  ...
  const { lang } = useLang();
  const t = adminUsersContent[lang];
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-32 text-gray-400">
        ...
        <p className="text-[10px] font-black uppercase   italic text-gray-300">{t.loading}</p>
      </div>
    );
  }
    ...
    <div className="w-full space-y-6">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 bg-white p-6 rounded-[2rem] border border-gray-100 shadow-sm">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-black text-gray-900   italic">{t.header.title}</h1>
            <p className="text-[10px] font-bold text-gray-400 uppercase  ">{t.header.subtitle}</p>
          </div>
        </div>
        ...
        <table className="w-full text-left hidden md:table">
          <thead>
            <tr className="bg-gray-50/50 border-b border-gray-100">
              <th className="p-6 text-[10px] font-black uppercase text-gray-400 w-16 text-center italic">{t.table.index}</th>
              <th className="p-6 text-[10px] font-black uppercase   text-gray-500 italic">{t.table.user}</th>
              <th className="p-6 text-[10px] font-black uppercase   text-gray-500 italic text-center">{t.table.joinDate}</th>
              <th className="p-6 text-[10px] font-black uppercase   text-gray-500 italic">{t.table.status}</th>
              <th className="p-6 text-[10px] font-black uppercase   text-gray-500 italic text-right">{t.table.actions}</th>
            </tr>
          </thead>
              ...
              <tr key={user._id} className="hover:bg-blue-50/10 transition-all group">
                ...
                <td className="p-6">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <p className="font-black text-gray-900 text-sm  ">{user.name || t.user.na}</p>
                ...
                <td className="p-6">
                  {user.isVerified ? (
                    <div className="flex items-center gap-2 text-emerald-500">
                      <BadgeCheck size={16} />
                      <span className="text-[10px] font-black uppercase  er">t.status.verified</span>
                    </div>
                  ) : (
                    <div className="flex items-center gap-2 text-amber-500">
                      <BadgeAlert size={16} className="animate-pulse" />
                      <span className="text-[10px] font-black uppercase  er">t.status.pending</span>
                    </div>
                  )}
                </td>
            ...
            <div key={user._id} className="p-5 bg-white border border-gray-100 rounded-3xl relative overflow-hidden shadow-sm">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-black text-gray-900 text-sm  ">{user.name || t.user.unnamed}</h3>
                    <p className="text-[10px] font-bold text-gray-400 italic">{t.user.entry} #{(currentPage - 1) * itemsPerPage + index + 1}</p>
                  </div>
                </div>
              ...
              <div className="flex gap-2">
                <button onClick={() => router.push(`/dashboard/user/${user._id}`)} className="flex-1 py-3 bg-gray-50 text-gray-900 rounded-2xl text-[10px] font-black uppercase border border-gray-100 flex items-center justify-center gap-2">
                  <UserCircle2 size={14} /> {t.actions.profile}
                </button>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between bg-white p-5 rounded-[2rem] border border-gray-100 shadow-sm gap-4">
        <p className="text-[10px] font-black text-gray-400 uppercase   italic">
          {t.pagination.audit}: {paginatedData.length} {t.pagination.of} {filteredUsers.length} {t.pagination.entries}
        </p>
