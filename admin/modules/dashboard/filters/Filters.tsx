...
export default function Filters({ filter, setFilter }: any) {
  ...
  const content = filtersContent[lang] || filtersContent.en;

  const options = [
    { id: "today", label: content.today, icon: <Clock size={14} /> },
    { id: "week", label: content.week, icon: <CalendarDays size={14} /> },
    { id: "month", label: content.month, icon: <CalendarRange size={14} /> },
  ];
