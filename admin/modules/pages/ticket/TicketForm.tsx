...
import { ticketFormContent } from "./ticketForm.content";
...
export default function TicketForm({
    ...
    const { lang } = useLang();
    const t = ticketFormContent[lang] ?? ticketFormContent.en;
    ...
    const handleSubmit = async () => {
        try {
            if (!visitId) {
                toast.error(t.errors.invalidVisit);
                return;
            }
            ...
            if (!formData.mobile || formData.mobile.length !== 10) {
                toast.error(t.errors.invalidMobile);
                return;
            }
            ...
            if (!otpVerified) {
                toast.error(t.errors.verifyOtp);
                return;
            }
        ...
        <div className="max-w-2xl mx-auto space-y-6 pb-10">
            ...
            <div className="bg-white/80 backdrop-blur-md p-6 rounded-[2rem] border border-gray-100 shadow-sm flex items-center gap-4">
                ...
                <div>
                    ...
                    <p className="text-xs font-bold text-gray-400 uppercase  ">{t.header.subtitle}</p>
                </div>
            </div>
            ...
            <div className="bg-white p-6 md:p-8 rounded-[2.5rem] border border-gray-100 shadow-sm space-y-6">
                ...
                {ticketFor === "OTHER" && (
                    <div className="space-y-3">
                        <label className="text-[10px] font-black uppercase ml-2 text-gray-400  ">{t.fields.beneficiaryIdentity}</label>
                        ...
                        <div className="flex items-center gap-2 ml-2">
                            {userFound || otpVerified ? (
                                <span className="flex items-center gap-1.5 text-[10px] font-black text-emerald-600 uppercase italic">
                                    <CheckCircle2 size={12} />{userFound ? t.fields.existingUser : t.fields.verifiedUser}
                                </span>
                            ) : (
                                <span className="flex items-center gap-1.5 text-[10px] font-black text-amber-500 uppercase italic">
                                    <ShieldCheck size={12} /> {t.fields.newUser}
                                </span>
                            )}
                        </div>
                    </div>
                ...
                <div className="space-y-2">
                    <label className="text-[10px] font-black uppercase ml-2 text-gray-400  ">{t.fields.fullName}</label>
                    <div className="relative group">
                ...
                <div className="space-y-2">
                    <label className="text-[10px] font-black uppercase ml-2 text-gray-400  ">{t.fields.department}</label>
                    <div className="relative">
                        <select
                        ...
                        >
                            <option value="">{t.fields.chooseDepartment}</option>
                            <option value="bpl">{t.departments.bpl}</option>
                            <option value="pmawas">{t.departments.pmawas}</option>
                            <option value="medical">{t.departments.medical}</option>
                            <option value="anudam">{t.departments.anudam}</option>
                        </select>
                ...
                {/* Description */}
                <div className="space-y-2">
                    <label className="text-[10px] font-black uppercase ml-2 text-gray-400  ">{t.fields.description}</label>
                    <textarea
                ...
                <div className="space-y-4">
                    <label className="text-[10px] font-black uppercase ml-2 text-gray-400  ">{t.fields.attachments}</label>
                    <div className="flex gap-3 flex-wrap">
                        ...
                        <label className="w-20 h-20 rounded-xl border-2 border-dashed border-gray-200 flex flex-col items-center justify-center text-gray-400 hover:text-blue-500 hover:border-blue-200 hover:bg-blue-50 transition-all cursor-pointer">
                            <Upload size={20} />
                            <span className="text-[8px] font-black uppercase mt-1">{t.fields.add}</span>
                ...
                otpSent && (
                    <div className="bg-amber-50 p-6 rounded-[2rem] border border-amber-100 space-y-4 shadow-sm animate-in zoom-in-95 duration-300">
                        <div className="flex items-center gap-2 text-amber-700">
                            <ShieldCheck size={20} />
                            <h4 className="font-black text-xs uppercase  ">{t.otp.title}</h4>
                        </div>
                        ...
                        {!otpSent ? (
                            ...
                            <div className="flex items-center justify-center gap-2 py-2 text-emerald-600 font-black text-xs uppercase   italic bg-white/50 rounded-xl">
                                <CheckCircle2 size={16} /> {t.otp.verified}
                            </div>
                        )}
                    </div>
                )}
