        ...
        {timeline.map((item: any, index: number) => {
          ...
          return (
            <div key={index} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group">
              ...
              {/* ICON */}
              <div className="flex items-center justify-center w-10 h-10 rounded-full border border-white shadow-md bg-white text-indigo-500 z-10 shrink-0 md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2 group-hover:scale-110 transition">
                {isVisit
              ...
              {/* CARD */}
              <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] bg-white p-5 rounded-3xl border border-gray-100 shadow-sm group-hover:shadow-md transition group-hover:-translate-y-1">
                ...
                {/* TITLE */}
                <h3 className="text-sm font-black text-gray-800 uppercase italic group-hover:text-indigo-600 transition">
                  {isVisit
                    ? item.data.purpose
                    : (item.data.subject || `#${item.data.ticketNumber}`)}
                </h3>
                ...
                <div className="mt-3 flex items-center justify-between border-t border-gray-50 pt-3">
                  <div className="flex items-center gap-1.5 text-[10px] font-black text-gray-400 uppercase">
                    {t.status}:{" "}
                    <span className={isVisit ? "text-emerald-500" : "text-blue-500"}>
                      {item.data.status}
                    </span>
                  </div>
